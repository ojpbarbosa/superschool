...
export default function Alumni() {
  ...
  function handleSubmit(event: FormEvent) {
    ...
    const method = newAlumni.id ? 'put' : 'post'
    const url = `https://localhost:7111/api/students/${
      method === 'put' ? newAlumni.id : ''
    }`
  ...
  function getUpdatedAlumni(newAlumni: typeof initialState.newAlumni) {
  ...
  function handleDelete(deletedAlumni: typeof initialState.newAlumni) {
    if (
      ...
      axios
        .delete(`https://localhost:7111/api/students/${deletedAlumni.id}`)
        .then(() => setAlumni(alumni.filter((a) => a.id !== deletedAlumni.id)))
    }
  }
